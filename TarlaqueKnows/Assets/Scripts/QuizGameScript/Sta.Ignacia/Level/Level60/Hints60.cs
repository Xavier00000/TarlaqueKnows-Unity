using UnityEngine;
public class Hints60 : MonoBehaviour
{
    public GameObject btnChoiParent;
    ButtonChoices60 btnChoiScript;
    public GameObject btnConParent;
    ButtonContainer60 btnConScript;
    int hint1 = 0;
    string[] answer = { "MADRIAGA" };
    ...
    void Start()
    {
        btnChoiScript = btnChoiParent.GetComponent<ButtonChoices60>();
        btnConScript = btnConParent.GetComponent<ButtonContainer60>();
    ...
    public void HintClicked01()
    {
        ...
        {            
            for(int ctr=0; ctr<btnConScript.btnContainer.Length; ctr++)
            {
                ...
                {
                    if(btnConScript.btnConText[ctr].text== "MADRIGALEJO")
                    {
            ...
            {
                if(btnChoiScript.btnChoiText[ctr].text== "MADRIGALEJO")
                {
        ...
        else if(hint1==1)
        {
            for(int ctr=0; ctr<btnConScript.btnContainer.Length; ctr++)
            {
                if(btnConScript.btnConText[ctr].text== "MADRIA")
                {
                    btnConScript.btnConText[ctr].text="";
                }
            ...
            for(int ctr=0; ctr<btnChoiScript.btnChoices.Length; ctr++)
            {
                if(btnChoiScript.btnChoiText[ctr].text== "MADRIA")
                {
                    btnChoiScript.btnChoices[ctr].SetActive(false);
                }
