using UnityEngine;
public class Hints52 : MonoBehaviour
{
    public GameObject btnChoiParent;
    ButtonChoices52 btnChoiScript;
    public GameObject btnConParent;
    ButtonContainer52 btnConScript;
    int hint1 = 0;
    string[] answer = { "SAN CLEMENTE DE ROMA" };
    ...
    void Start()
    {
        btnChoiScript = btnChoiParent.GetComponent<ButtonChoices52>();
        btnConScript = btnConParent.GetComponent<ButtonContainer52>();
    ...
    public void HintClicked01()
    {
        ...
        {            
            for(int ctr=0; ctr<btnConScript.btnContainer.Length; ctr++)
            {
                ...
                {
                    if(btnConScript.btnConText[ctr].text== "MONCADA DE ROMA")
                    {
            ...
            {
                if(btnChoiScript.btnChoiText[ctr].text== "MONCADA DE ROMA")
                {
        ...
        else if(hint1==1)
        {
            for(int ctr=0; ctr<btnConScript.btnContainer.Length; ctr++)
            {
                if(btnConScript.btnConText[ctr].text== "SAN JOSE DE ROMA")
                {
                    btnConScript.btnConText[ctr].text="";
                }
            ...
            for(int ctr=0; ctr<btnChoiScript.btnChoices.Length; ctr++)
            {
                if(btnChoiScript.btnChoiText[ctr].text== "SAN JOSE DE ROMA")
                {
                    btnChoiScript.btnChoices[ctr].SetActive(false);
                }
