using UnityEngine;
public class Hints30 : MonoBehaviour
{
    public GameObject btnChoiParent;
    ButtonChoices30 btnChoiScript;
    public GameObject btnConParent;
    ButtonContainer30 btnConScript;
    int hint1 = 0;
    string[] answer = { "MONASTERIO DE TARLAC" };
    ...
    void Start()
    {
        btnChoiScript = btnChoiParent.GetComponent<ButtonChoices30>();
        btnConScript = btnConParent.GetComponent<ButtonContainer30>();
    ...
    public void HintClicked01()
    {
        ...
        if (hint1 == 0)
        {
            for (int ctr = 0; ctr < btnConScript.btnContainer.Length; ctr++)
            {
                ...
                {
                    if (btnConScript.btnConText[ctr].text == "LIMIAC PARK")
                    {
            ...
            {
                if (btnChoiScript.btnChoiText[ctr].text == "LIMIAC PARK")
                {
        ...
        else if (hint1 == 1)
        {
            for (int ctr = 0; ctr < btnConScript.btnContainer.Length; ctr++)
            {
                if (btnConScript.btnConText[ctr].text == "SAIPAN BEACH")
                {
                    btnConScript.btnConText[ctr].text = "";
                }
            ...
            for (int ctr = 0; ctr < btnChoiScript.btnChoices.Length; ctr++)
            {
                if (btnChoiScript.btnChoiText[ctr].text == "SAIPAN BEACH")
                {
                    btnChoiScript.btnChoices[ctr].SetActive(false);
                }
