...
public class Hints21 : MonoBehaviour
{
    public GameObject btnChoiParent;
    ButtonChoices21 btnChoiScript;
    public GameObject btnConParent;
    ButtonContainer21 btnConScript;
    int hint1 = 0;
    string[] answer = {"S","T","J","O","S","E","M","A","R","I","A"};
    bool[] isAlreadyAnswer = new bool[16];
    ...
    void Start()
    {
        btnChoiScript = btnChoiParent.GetComponent<ButtonChoices21>();
        btnConScript = btnConParent.GetComponent<ButtonContainer21>();
    ...
    public void HintClicked01()
    {
        ...
        {            
            for(int ctr=0; ctr<btnConScript.btnContainer.Length; ctr++)
            {
                for(int ctr2=0; ctr2<btnChoiScript.btnChoices.Length; ctr2++)
                {
                    ...
                    if (btnConScript.btnConText[ctr].text == "U")
                    {
                        btnConScript.btnConText[ctr].text = "";
                    }
            ...
            for(int ctr=0; ctr<btnChoiScript.btnChoices.Length; ctr++)
            {
                ...
                if (btnChoiScript.btnChoiText[ctr].text == "U")
                {
                    btnChoiScript.btnChoices[ctr].SetActive(false);
                }
        ...
        else if(hint1==1)
        {
            for(int ctr=0; ctr<btnConScript.btnContainer.Length; ctr++)
            {
                if(btnConScript.btnConText[ctr].text=="Q")
                {
                    btnConScript.btnConText[ctr].text="";
                }
            ...
            for(int ctr=0; ctr<btnChoiScript.btnChoices.Length; ctr++)
            {
                if(btnChoiScript.btnChoiText[ctr].text=="Q")
                {
                    btnChoiScript.btnChoices[ctr].SetActive(false);
                }
                ...
                btnConScript.btnConText[ctr].text=="")
            {
                ...
                for(int ctr2=0; ctr2<btnChoiScript.btnChoices.Length; ctr2++)
                {
                    if(btnConScript.btnConText[ctr].text==btnChoiScript.btnChoiText[ctr2].text)//is current answer text == to current btn choices text
                    {
                        ...
                        {
                            bool[] activeState = new bool[5];
                            int activeStateIndex=0;
                            int[] BtnLocationIndex = new int[5];
